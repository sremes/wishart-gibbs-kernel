function [model] = up_w_b_ridge(model)

one = ones(model.S,1);

Sigma_inv = [model.uu + model.lambda_w*eye(size(model.uu)), model.u * one;
...
a0 = 1e-6;
model.lambda0 = a0;
model.lambda_w_alpha = a0 + 0.5*length(model.w);
model.lambda_w_beta = a0 + 0.5*sum(model.w.^2 + diag(model.wb_sigma(1:end-1,1:end-1)));
